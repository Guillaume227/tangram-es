...
void TangramPlatformAdapter::requestRender() {
	return tangramHelper->requestRender();
}

std::string TangramPlatformAdapter::fontFallbackPath(int importance, int weightHint) {
	return tangramHelper->getFontFallbackFilePath(importance, weightHint);
}

std::string TangramPlatformAdapter::fontPath(const std::string& _family, const std::string& _weight, const std::string& _style) {
...
std::vector<FontSourceHandle> TangramPlatformAdapter::systemFontFallbacksHandle() {
...
std::vector<char> TangramPlatformAdapter::systemFont(const std::string& _name, const std::string& _weight, const std::string& _face) {
...
bool TangramPlatformAdapter::isContinuousRendering() {
    return tangramHelper->isContinuousRendering();
}

bool TangramPlatformAdapter::bytesFromAssetManager(const char* _path, std::function<char*(size_t)> _allocator) {
...
std::string TangramPlatformAdapter::stringFromFile(const char* _path) {
...
std::vector<char> TangramPlatformAdapter::bytesFromFile(const char* _path) {
...
bool TangramPlatformAdapter::startUrlRequest(const std::string& url, UrlCallback _callback) {

	return tangramHelper->startUrlRequest(url, std::make_shared<CUrlCallback>(_callback));
}

void TangramPlatformAdapter::cancelUrlRequest(const std::string& url) {
